...
class Shoulder:
    def __init__(self, open_pin: int, close_pin: int):
        self.__open_pwm = PwmPin(open_pin, __FREQUENCY)
        self.__close_pwm = PwmPin(close_pin, __FREQUENCY)

    async def open(self):
        self.__open_pwm.set_dc(50)
        await asyncio.sleep((1 / __FREQUENCY) * 400)
...
class Hand:
    ...
    async def __init__(
        ...
        await self.__set_angle(release_angle)

...
def find_path(stage: Stage, start: tuple[int, int], end: tuple[int, int]):
    """
...
def create_grid_map(stage: Stage, cell_size=250):
    """
    ...
    Returns:
        tuple[np.ndarray, int] | tuple[None, None]: グリッドマップとセルサイズ、または作成に失敗した場合はNone
    """
    if stage.robot is None:
        return None, None
...
def a_star_search(
    ...
    Args:
        ...
        start_pos (tuple[float, float]): スタート地点の座標 (x, y)
        end_pos (tuple[float, float]): ゴール地点の座標 (x, y)
        cell_size (int): セルサイズ

    Returns:
        list[tuple[float, float]] | None: 経路のリスト、または経路が見つからない場合はNone
    """
...
def reconstruct_path(came_from, current, cell_size):
    """経路を再構築する関数"""
    path = []
...
def smooth_path(path: np.ndarray):
    ...
    new_points = splev(np.linspace(0, 1, 100), tuple(tck))  # type: ignore
